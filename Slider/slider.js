...
(function(_) {
    ...
    var cursorTemplate = '<ul class="m-cursor"><li class="prev">&lt;</li><li class="next">&gt;</li></ul>';
    ...
    _.extend(Slider.prototype, {
        ...
        _initCursor: function() {
            ...
            this.cursors = [];
            //添加前后翻页
            var prev = this.m_cursor.firstChild,
                next = this.m_cursor.lastChild;
            addEvent(prev, 'click', this.prev.bind(this));
            addEvent(next, 'click', this.next.bind(this));

            for (var i = 0; i < this.pageNum; i++) {
                var cursor = document.createElement('li');
                cursor.className = 'cursor';
                cursor.textContent = i + 1;
        ...
        _upDateCursor: function() {
            for (var i = 0; i < this.pageNum; i++) {
                if (this.pageIndex === i) _.addClass(this.cursors[i], 'active');
                else _.delClass(this.cursors[i], 'active');
            }
        },
        //清除选区
        _clearSelection() {
    ...
    _.extend(Slider.prototype, {
        ...
        setAutoPlay: function() {
            if (!this.intervalID) this.intervalID = setInterval(this.next.bind(this), 3000);
        },
